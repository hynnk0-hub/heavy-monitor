// src/components/VibTrendChart.jsx
...
export function VibTrendChart({
  ...
  yMin,
  yMax,
  // 옵션: 기준선(채우는 바닥). 기본은 0. yMin까지 채우고 싶으면 "yMin" 전달.
  base = "zero", // "zero" | "yMin" | number | "dataMin"
  isAnimationActive = false,
  seriesName,
}) {
  // 안전한 고유 ID(그라디언트 충돌 방지)
  ...
  if (hasTs) {
    ...
    const windowSize = avgDelta * Math.max(20, data.length); // 최근 20포인트 기준
    domainMax = lastTs;
